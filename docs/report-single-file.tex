...
\begin{spacing}{1.5}
    ...
    \begin{figure}[H]
        ...
        \resizebox{\textwidth}{!}{
        \begin{tikzpicture} [
            actor/.style={draw, circle, minimum size=1.5cm, text centered, font=\small\bfseries},
            usecase/.style={draw, ellipse, minimum width=2.5cm, minimum height=1cm, text centered, font=\small},
            group/.style={draw, rectangle, rounded corners, dashed, inner sep=10pt},
            line/.style={draw, -}
        ]
            % Actors on left side
            \node[actor] (admin) at (-7,3) {Admin};
            \node[actor] (engineer) at (-7,0) {Engineer};
            \node[actor] (auditor) at (-7,-3) {Auditor};
            
            % Actors on right side
            \node[actor] (ulb) at (7,3) {ULB Official};
            \node[actor] (agency) at (7,0) {Agency Official};
            \node[actor] (safety) at (7,-3) {Safety Officer};
            
            % Progress Use Cases
            \node[group, minimum width=3.5cm, minimum height=6cm] (progGroup) at (-2.5,0) {};
            \node[font=\small\bfseries] at (-2.5,3) {Construction Progress Use Cases};
            \node[usecase] (upload) at (-2.5,2) {Upload Progress \textrm{Images}};
            \node[usecase] (classify) at (-2.5,0.5) {Classify \textrm{Construction Stage}};
            \node[usecase] (compare) at (-2.5,-1) {Compare Progress};
            \node[usecase] (manageProj) at (-2.5,-2.5) {Manage Projects};
            
            % Safety Use Cases
            \node[group, minimum width=3.5cm, minimum height=4cm] (safetyGroup) at (2.5,-2) {};
            \node[font=\small\bfseries] at (2.5,-0) {Worker Safety Use Cases};
            \node[usecase] (uploadSafety) at (2.5,-1) {Upload Safety \textrm{Images}};
            \node[usecase] (detectPPE) at (2.5,-2.5) {Detect PPE \textrm{Compliance}};
            \node[usecase] (safetyReport) at (2.5,-4) {Generate Safety \textrm{Reports}};
            
            % General Use Cases
            \node[group, minimum width=3.5cm, minimum height=3cm] (genGroup) at (2.5,2) {};
            \node[font=\small\bfseries] at (2.5,3.5) {General Use Cases};
            \node[usecase] (users) at (2.5,3) {Manage Users};
            \node[usecase] (reports) at (2.5,1) {Generate Reports};
            \node[usecase] (dashboard) at (0,4) {View Dashboard};
            
            % Admin connections
            \draw[line] (admin) -- (users);
            \draw[line] (admin) -- (manageProj);
            \draw[line] (admin) -- (dashboard);
            
            % Engineer connections
            \draw[line] (engineer) -- (upload);
            \draw[line] (engineer) -- (classify);
            \draw[line] (engineer) -- (compare);
            \draw[line] (engineer) -- (uploadSafety);
            \draw[line] (engineer) -- (detectPPE);
            \draw[line] (engineer) -- (dashboard);
            
            % Auditor connections
            \draw[line] (auditor) -- (compare);
            \draw[line] (auditor) -- (reports);
            \draw[line] (auditor) -- (dashboard);
            
            % ULB Official connections
            \draw[line] (ulb) -- (manageProj);
            \draw[line] (ulb) -- (reports);
            \draw[line] (ulb) -- (dashboard);
            
            % Agency Official connections
            \draw[line] (agency) -- (reports);
            \draw[line] (agency) -- (dashboard);
            
            % Safety Officer connections
            \draw[line] (safety) -- (uploadSafety);
            \draw[line] (safety) -- (detectPPE);
            \draw[line] (safety) -- (safetyReport);
        \end{tikzpicture}
        }
        \caption{IBCM Use Case Diagram}
    ...
    A Class Diagram would model the main classes in the system (e.g., `User`, `Project`, `Image`, `AnalysisResult`, `PPE Detector`, `SSIMComparator`) and their attributes, methods, and relationships (associations, inheritance, dependencies). This helps visualize the static structure of the software.
    ...
    \begin{figure}[H]
        \centering
        % Construction Progress Sequence Diagram
        \resizebox{\textwidth}{!}{
        \begin{tikzpicture} [
            participant/.style={draw, rectangle, minimum width=2cm, minimum height=1cm, text centered, font=\small\bfseries},
            line/.style={draw, -}, 
            arrow/.style={-stealth, thick},
            note/.style={draw, rectangle, rounded corners, fill=yellow!20, text width=3cm, font=\footnotesize, text centered},
            sequence-node/.style={inner sep=2pt, font=\footnotesize},
            scale=0.85, transform shape
        ]
            % Participants
            \node[participant] (user) at (0,0) {User};
            \node[participant] (frontend) at (2.5,0) {Frontend};
            \node[participant] (api) at (5,0) {Backend API};
            \node[participant] (ml) at (7.5,0) {ML Model};
            \node[participant] (db) at (10,0) {Database};
            
            % Lifelines
            \foreach \x in {user,frontend,api,ml,db} {
                \draw[dashed] (\x) -- (\x |-0,-10);
            }
            
            % Sequence
            \draw[arrow] (user) -- node[sequence-node, above]{1. Upload previous \& current images} (frontend);
            \draw[arrow] (frontend) to[out=-10,in=190,looseness=1] node[sequence-node, below, yshift=-0.1cm]{2. Validate file format/size} (frontend);
            \draw[arrow] (frontend) -- node[sequence-node, above]{3. POST /api/ssim} (api);
            \draw[arrow] (api) -- node[sequence-node, above]{4. Process images using SSIM} (ml);
            \draw[arrow] (ml) to[out=-10,in=190,looseness=1] node[sequence-node, below, yshift=-0.1cm]{5. Calculate similarity \& changes} (ml);
            \draw[arrow] (ml) -- node[sequence-node, above]{6. Return comparison results} (api);
            \draw[arrow] (api) -- node[sequence-node, above]{7. Store progress log} (db);
            \draw[arrow] (api) -- node[sequence-node, above]{8. Return analysis results} (frontend);
            \draw[arrow] (frontend) -- node[sequence-node, above]{9. Display progress visualization} (user);
            
            % Title
            \node[font=\bfseries] at (5,1) {Construction Progress Sequence Diagram};
        \end{tikzpicture}
        }
        % Worker Safety Sequence Diagram
        \begin{tikzpicture} [
            participant/.style={draw, rectangle, minimum width=2cm, minimum height=1cm, text centered, font=\small\bfseries},
            line/.style={draw, -}, 
            arrow/.style={-stealth, thick},
            ...
            % Participants
            \node[participant] (user) at (0,-12) {User};
            \node[participant] (frontend) at (2.5,-12) {Frontend};
            \node[participant] (api) at (5,-12) {Backend API};
            \node[participant] (ml) at (7.5,-12) {YOLO Model};
            \node[participant] (db) at (10,-12) {Database};
            
            % Lifelines
            \foreach \x in {user,frontend,api,ml,db} {
                \draw[dashed] (\x) -- (\x |-12,-22);
            }
            
            % Sequence
            \draw[arrow] (user) -- node[sequence-node, above]{1. Upload safety image} (frontend);
            \draw[arrow] (frontend) to[out=-10,in=190,looseness=1] node[sequence-node, below, yshift=-0.1cm]{2. Validate file format/size} (frontend);
            \draw[arrow] (frontend) -- node[sequence-node, above]{3. POST /api/ppe-detection} (api);
            \draw[arrow] (api) -- node[sequence-node, above]{4. Process image with YOLO} (ml);
            \draw[arrow] (ml) to[out=-10,in=190,looseness=1] node[sequence-node, below, yshift=-0.1cm]{5. Detect workers and PPE items} (ml);
            \draw[arrow] (ml) -- node[sequence-node, above]{6. Return detection results} (api);
            \draw[arrow] (api) -- node[sequence-node, above]{7. Store safety compliance data} (db);
            \draw[arrow] (api) -- node[sequence-node, above]{8. Return annotated image \& counts} (frontend);
            \draw[arrow] (frontend) -- node[sequence-node, above]{9. Display safety compliance report} (user);
            
            % Title
            \node[font=\bfseries] at (5,-11) {Worker Safety Sequence Diagram};
        \end{tikzpicture}
    ...
    Key tables include:
    \begin{itemize}
        \item \textbf{`users`:} Stores user credentials, roles (using ENUM), and personal details.
        \item \textbf{`countries`, `states`, `cities`, `locations`:} Manage geographical information hierarchically, linked to projects.
        \item \textbf{`projects`:} Contains project details, status (ENUM), location foreign key, and creator foreign key.
        \item \textbf{`images`:} Stores metadata for uploaded images, including project/user links, URL, image type (ENUM: 'progress', 'safety'), activity type (ENUM), validation status, and timestamps.
        \item \textbf{`image\_analysis` / `PROGRESS\_LOGS}: Records results of SSIM comparisons between image pairs,including the respective images and storing scores/change descriptions.
        \item \textbf{`ppe\_types`, `ppe\_results`, `ppe\_detection`:} Manage PPE detection results. `ppe\_types` is a reference table. `ppe\_results` links to an image. `ppe\_detection` stores the count of each detected PPE type for a specific result.
        \item \textbf{`audit\_logs`:} Tracks user actions for security and accountability.
    \end{itemize}
    ...
    \begin{figure}[H]
        ...
        \resizebox{\textwidth}{!}{
        \begin{tikzpicture} [
            entity/.style={draw, rectangle, minimum width=2.5cm, minimum height=1cm, text centered, font=\small\bfseries, fill=blue!10},
            relationship/.style={draw, diamond, aspect=2, text centered, font=\footnotesize, fill=red!10},
            attribute/.style={draw, ellipse, minimum width=1.8cm, minimum height=0.8cm, text centered, font=\footnotesize, fill=green!10},
            line/.style={draw, -},
            onearrow/.style={draw, ->, >=stealth},
            manyarrow/.style={draw, -{Stealth[length=10pt, width=7pt]}, >=stealth},
            node distance=2cm
        ]
            % Core entities
            \node[entity] (users) at (0,0) {USERS};
            \node[entity] (projects) at (6,0) {PROJECTS};
            \node[entity] (images) at (12,0) {IMAGES};
            
            % Geography entities
            \node[entity] (countries) at (-4,4) {COUNTRIES};
            \node[entity] (states) at (-2,7) {STATES};
            \node[entity] (cities) at (2,7) {CITIES};
            \node[entity] (locations) at (0,4) {LOCATIONS};
            
            % Analysis entities
            \node[entity] (progLogs) at (9,-4) {PROGRESS\_LOGS};
            \node[entity] (imgAnalysis) at (6,-4) {IMAGE\_ANALYSIS};
            \node[entity] (ppeResults) at (14,-4) {PPE\_RESULTS};
            \node[entity] (ppeTypes) at (16,-6) {PPE\_TYPES};
            \node[entity] (ppeDetection) at (12,-6) {PPE\_DETECTION};
            \node[entity] (auditLogs) at (-3,-4) {AUDIT\_LOGS};
            
            % Relationships - Geography
            \draw[onearrow] (countries) -- (states) node[midway, above, sloped, font=\footnotesize] {has};
            \draw[onearrow] (states) -- (cities) node[midway, above, sloped, font=\footnotesize] {has};
            \draw[onearrow] (countries) -- (locations) node[midway, above, sloped, font=\footnotesize] {located\_in};
            \draw[onearrow] (states) -- (locations) node[midway, above, sloped, font=\footnotesize] {located\_in};
            \draw[onearrow] (cities) -- (locations) node[midway, above, sloped, font=\footnotesize] {located\_in};
            
            % Relationships - Core entities
            \draw[onearrow] (users) -- (projects) node[midway, above, font=\footnotesize] {creates};
            \draw[onearrow] (users) -- (images) node[midway, above, sloped, font=\footnotesize] {uploads};
            \draw[onearrow] (users) -- (auditLogs) node[midway, above, sloped, font=\footnotesize] {performs};
            \draw[onearrow] (locations) -- (projects) node[midway, above, sloped, font=\footnotesize] {located\_at};
            \draw[onearrow] (projects) -- (images) node[midway, above, font=\footnotesize] {contains};
            \draw[onearrow] (projects) -- (progLogs) node[midway, above, sloped, font=\footnotesize] {tracks};
            \draw[onearrow] (projects) -- (imgAnalysis) node[midway, above, sloped, font=\footnotesize] {analyzes};
            
            % Relationships - Analysis entities
            \draw[onearrow] (images) -- (ppeResults) node[midway, above, sloped, font=\footnotesize] {analyzed\_for};
            \draw[onearrow] (images) to[out=270,in=135] (progLogs) node[midway, above, sloped, font=\footnotesize] {previous};
            \draw[onearrow] (images) to[out=315,in=45] (progLogs) node[midway, below, sloped, font=\footnotesize] {current};
            \draw[onearrow] (ppeResults) -- (ppeDetection) node[midway, left, font=\footnotesize] {contains};
            \draw[onearrow] (ppeTypes) -- (ppeDetection) node[midway, above, font=\footnotesize] {categorizes};
            
            % Key attributes for USERS
            \node[attribute] (userId) at (-2.5,-1.5) {id (PK)};
            \node[attribute] (userName) at (-1,-2.5) {name};
            \node[attribute] (userRole) at (0,-2) {role};
            \node[attribute] (userEmail) at (1,-2.5) {email};
            \draw[line] (users) -- (userId);
            \draw[line] (users) -- (userName);
            \draw[line] (users) -- (userRole);
            \draw[line] (users) -- (userEmail);
            
            % Key attributes for PROJECTS
            \node[attribute] (projId) at (4.5,-1.5) {id (PK)};
            \node[attribute] (projName) at (6,-2) {name};
            \node[attribute] (projStatus) at (7.5,-1.5) {status};
            \draw[line] (projects) -- (projId);
            \draw[line] (projects) -- (projName);
            \draw[line] (projects) -- (projStatus);
            
            % Key attributes for IMAGES
            \node[attribute] (imgId) at (10.5,-1.5) {id (PK)};
            \node[attribute] (imgUrl) at (12,-2) {image\_url};
            \node[attribute] (imgType) at (13.5,-1.5) {image\_type};
            \draw[line] (images) -- (imgId);
            \draw[line] (images) -- (imgUrl);
            \draw[line] (images) -- (imgType);
            
            % Key attributes for PPE_RESULTS and PROGRESS_LOGS
            \node[attribute] (ppeId) at (14,-5.5) {id (PK)};
            \node[attribute] (progId) at (9,-5.5) {id (PK)};
            \node[attribute] (progScore) at (9,-7) {ssim\_score};
            \draw[line] (ppeResults) -- (ppeId);
            \draw[line] (progLogs) -- (progId);
            \draw[line] (progLogs) -- (progScore);
        \end{tikzpicture}
        }
        \caption{IBCM Entity Relationship Diagram (Simplified)}
...
\end{listing}
    ...
    \begin{figure}[H]
        \centering
        \fbox{Placeholder for Dashboard Screenshot}
    ...
    \begin{figure}[H]
        \centering
        \fbox{Placeholder for Image Comparison UI Screenshot}
    ...
    \begin{figure}[H]
        \centering
        \fbox{Placeholder for PPE Detection UI Screenshot}
    ...
    When an image is processed by the `/api/ppe-detection` endpoint, the system identifies objects belonging to classes such as 'Person', 'Hardhat', 'NO-Hardhat', 'Safety Vest', 'NO-Safety Vest', 'Mask', 'NO-Mask', etc. Figure \ref{fig:ppe_example} shows an example output with bounding boxes and labels.
    ...
    Initial tests were conducted to measure the processing time for the core analysis functions.
    \begin{itemize}
        \item \textbf{SSIM Analysis:} The time taken for the `/api/ssim` endpoint depends on image size, whether alignment is needed, and server resources. Preliminary tests on [Specify Test Environment, e.g., local machine with CPU/GPU] showed average processing times of [X] seconds per image pair.
        \item \textbf{PPE Detection:} The time taken for the `/api/ppe-detection` endpoint depends largely on the model complexity and hardware (CPU vs. GPU). On the same test environment, average inference time was [Y] seconds per image.
    \end{itemize}
    These times are generally within the target non-functional requirement (< 30 seconds), but further optimization and testing under load are recommended.
...
\begin{thebibliography}{99}
    \bibitem{SRD_IBCM} IBCM Development Team. (2024). *Software Requirements Document: IBCM Image Based Construction Monitoring*. Version 1.1.
    ...
    \bibitem{CNN_Construction} [Add a relevant citation for CNNs in construction stage classification]
    \bibitem{PPE_Detection} [Add a relevant citation for ML-based PPE detection]
